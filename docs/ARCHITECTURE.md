# PromptXY 完整架构设计

> **版本**: v2.0 (前后端分离版)
> **创建日期**: 2025-12-20
> **状态**: ✅ 已实现 (2025-12-21)

---

## 🏗️ 项目结构

```
promptxy/
├── backend/                      # 后端服务 (Node.js + TypeScript)
│   ├── src/
│   │   ├── main.ts              # 应用入口
│   │   ├── gateway.ts           # HTTP代理核心 (端口: 7070)
│   │   ├── api-server.ts        # API服务器 (端口: 7071)
│   │   ├── database.ts          # SQLite数据库操作
│   │   ├── config.ts            # 配置文件管理
│   │   ├── types.ts             # TypeScript类型定义
│   │   └── rules/
│   │       └── engine.ts        # 规则引擎 (已有)
│   │   └── adapters/
│   │       ├── claude.ts        # Claude适配器 (已有)
│   │       ├── codex.ts         # Codex适配器 (已有)
│   │       └── gemini.ts        # Gemini适配器 (已有)
│   ├── package.json
│   ├── tsconfig.json
│   └── .env.example
│
├── frontend/                     # 前端应用 (React + HeroUI)
│   ├── src/
│   │   ├── main.tsx             # 应用入口
│   │   ├── App.tsx              # 主组件
│   │   ├── components/
│   │   │   ├── layout/          # 布局组件
│   │   │   │   ├── Header.tsx
│   │   │   │   ├── Sidebar.tsx
│   │   │   │   └── DetailPanel.tsx
│   │   │   ├── rules/           # 规则相关
│   │   │   │   ├── RuleList.tsx
│   │   │   │   ├── RuleEditor.tsx
│   │   │   │   └── RuleCard.tsx
│   │   │   ├── requests/        # 请求相关
│   │   │   │   ├── RequestList.tsx
│   │   │   │   ├── RequestDetail.tsx
│   │   │   │   └── DiffViewer.tsx
│   │   │   ├── preview/         # 预览相关
│   │   │   │   └── PreviewPanel.tsx
│   │   │   └── common/          # 通用组件
│   │   │       ├── StatusIndicator.tsx
│   │   │       ├── Modal.tsx
│   │   │       └── EmptyState.tsx
│   │   ├── pages/
│   │   │   ├── RulesPage.tsx
│   │   │   ├── RequestsPage.tsx
│   │   │   ├── PreviewPage.tsx
│   │   │   └── SettingsPage.tsx
│   │   ├── hooks/
│   │   │   ├── useRules.ts
│   │   │   ├── useRequests.ts
│   │   │   ├── useSSE.ts
│   │   │   └── useConfig.ts
│   │   ├── api/
│   │   │   ├── client.ts        # API客户端
│   │   │   ├── rules.ts         # 规则API
│   │   │   ├── requests.ts      # 请求API
│   │   │   └── sse.ts           # SSE客户端
│   │   ├── store/
│   │   │   ├── index.ts         # Zustand store
│   │   │   ├── app-store.ts     # 应用状态
│   │   │   └── ui-store.ts      # UI状态
│   │   ├── types/
│   │   │   ├── index.ts         # 导出所有类型
│   │   │   ├── rule.ts          # 规则类型
│   │   │   ├── request.ts       # 请求类型
│   │   │   └── api.ts           # API类型
│   │   ├── utils/
│   │   │   ├── formatter.ts     # 格式化工具
│   │   │   ├── validator.ts     # 验证工具
│   │   │   └── diff.ts          # 差异对比算法
│   │   └── styles/
│   │       ├── globals.css      # 全局样式
│   │       └── components.css   # 组件样式
│   ├── public/
│   │   └── index.html
│   ├── package.json
│   ├── vite.config.ts
│   ├── tsconfig.json
│   ├── tailwind.config.js       # HeroUI需要
│   └── postcss.config.js
│
├── scripts/                      # 部署脚本
│   ├── build.sh                 # 构建脚本
│   ├── start.sh                 # 启动脚本
│   └── dev.sh                   # 开发脚本
│
├── docs/                         # 文档
│   ├── ARCHITECTURE.md          # 此文件
│   ├── BACKEND_API_EXTENSION.md # 后端API
│   ├── FRONTEND_SETUP.md        # 前端配置
│   └── DEPLOYMENT.md            # 部署指南
│
├── .gitignore
├── README.md
└── package.json                 # 根级，用于一键启动
```

---

## 🎯 核心设计原则

### 1. 前后端分离但统一部署

- **开发时**：独立开发，独立启动
- **部署时**：合并为一个包，统一管理
- **通信**：HTTP API + SSE实时推送

### 2. 数据流设计

```
┌─────────────────────────────────────────────────────────────┐
│                         用户操作                             │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ↓
┌─────────────────────────────────────────────────────────────┐
│                    前端 (React + HeroUI)                     │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │  规则管理    │  │  请求查看    │  │  预览测试    │     │
│  │  (UI层)      │  │  (UI层)      │  │  (UI层)      │     │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘     │
│         │                 │                 │              │
│         └────────┬────────┴────────┬────────┘              │
│                  │                 │                       │
│         ┌────────▼────────┐  ┌─────▼──────┐               │
│         │  API客户端      │  │  SSE客户端 │               │
│         │  (fetch/axios)  │  │  (EventSource)│             │
│         └────────┬────────┘  └─────┬──────┘               │
└──────────────────┼──────────────────┼──────────────────────┘
                   │                  │
                   ↓                  ↓
┌─────────────────────────────────────────────────────────────┐
│                    后端 (Node.js + TS)                       │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │  API服务器   │  │  SSE推送     │  │  代理核心    │     │
│  │  (端口7071)  │  │  (端口7071)  │  │  (端口7070)  │     │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘     │
│         │                 │                 │              │
│         └────────┬────────┴────────┬────────┘              │
│                  │                 │                       │
│         ┌────────▼────────┐  ┌─────▼──────┐               │
│         │  SQLite数据库   │  │  配置文件  │               │
│         │  (请求历史)     │  │  (规则)    │               │
│         └─────────────────┘  └────────────┘               │
└─────────────────────────────────────────────────────────────┘
                   │
                   ↓
┌─────────────────────────────────────────────────────────────┐
│                    CLI工具 (Claude/Codex/Gemini)             │
│  发起请求 → 经过代理(7070) → 规则修改 → 转发上游API          │
└─────────────────────────────────────────────────────────────┘
```

---

## 📡 API通信协议

### 后端API端点（端口7071）

#### 1. 规则管理

```
GET    /api/rules              # 获取所有规则
POST   /api/rules              # 创建规则
GET    /api/rules/:id          # 获取单个规则
PUT    /api/rules/:id          # 更新规则
DELETE /api/rules/:id          # 删除规则
POST   /api/rules/batch        # 批量操作
```

#### 2. 请求历史

```
GET    /api/requests           # 获取请求列表（分页、筛选）
GET    /api/requests/:id       # 获取请求详情
POST   /api/requests/cleanup   # 清理旧数据
DELETE /api/requests/:id       # 删除单个请求
```

#### 3. 实时推送

```
GET    /api/events             # SSE连接（新请求推送）
```

#### 4. 配置管理

```
GET    /api/config             # 获取配置
POST   /api/config/sync        # 同步配置（立即生效）
POST   /api/config/export      # 导出配置
POST   /api/config/import      # 导入配置
```

#### 5. 预览测试

```
POST   /api/preview            # 预览规则效果
```

### SSE事件类型

```typescript
// 新请求事件
event: request
data: {
  "id": "req-20251220-143215-abc",
  "timestamp": 1734705135000,
  "client": "codex",
  "path": "/v1/chat/completions",
  "method": "POST"
}

// 配置更新事件（可选）
event: config_updated
data: {
  "timestamp": 1734705135000,
  "ruleCount": 42
}
```

---

## 🔧 技术栈详解

### 后端技术栈

```json
{
  "dependencies": {
    "express": "^4.18.2", // Web框架
    "sqlite3": "^5.1.6", // SQLite数据库
    "sqlite": "^4.1.2", // SQLite TypeScript封装
    "cors": "^2.8.5" // 跨域支持
  },
  "devDependencies": {
    "@types/node": "^20.10.0",
    "@types/express": "^4.17.21",
    "@types/cors": "^2.8.17",
    "typescript": "^5.3.2",
    "ts-node": "^10.9.1",
    "nodemon": "^3.0.2"
  }
}
```

### 前端技术栈

```json
{
  "dependencies": {
    "react": "^18.2.0",
    "react-dom": "^18.2.0",
    "@heroui/react": "^2.2.0", // HeroUI组件库
    "@heroui/system": "^2.2.0",
    "framer-motion": "^10.16.0", // 动画库
    "zustand": "^4.4.0", // 状态管理
    "@tanstack/react-query": "^5.0.0", // 数据获取
    "axios": "^1.6.0" // HTTP客户端
  },
  "devDependencies": {
    "@types/react": "^18.2.0",
    "@types/react-dom": "^18.2.0",
    "@vitejs/plugin-react": "^4.2.0",
    "typescript": "^5.3.2",
    "vite": "^5.0.0",
    "tailwindcss": "^3.3.0",
    "postcss": "^8.4.31",
    "autoprefixer": "^10.4.16"
  }
}
```

---

## 📊 数据模型

### 1. 规则 (Rule)

```typescript
interface Rule {
  id: string; // 唯一标识
  description?: string; // 描述

  when: {
    client: 'claude' | 'codex' | 'gemini';
    field: 'system' | 'instructions';
    method?: string;
    pathRegex?: string;
    modelRegex?: string;
  };

  ops: Array<
    | { type: 'set'; text: string }
    | { type: 'append'; text: string }
    | { type: 'prepend'; text: string }
    | { type: 'replace'; match?: string; regex?: string; flags?: string; replacement: string }
    | { type: 'delete'; match?: string; regex?: string; flags?: string }
    | { type: 'insert_before'; regex: string; flags?: string; text: string }
    | { type: 'insert_after'; regex: string; flags?: string; text: string }
  >;

  stop?: boolean; // 是否在此后停止
  enabled: boolean; // 是否启用
  createdAt: number; // 创建时间
  updatedAt: number; // 更新时间
}
```

### 2. 请求记录 (RequestRecord)

```typescript
interface RequestRecord {
  id: string;
  timestamp: number;

  // 请求信息
  client: string;
  path: string;
  method: string;

  // 请求体
  originalBody: any; // 原始请求
  modifiedBody: any; // 修改后请求

  // 规则匹配
  matchedRules: Array<{
    ruleId: string;
    opType: string;
  }>;

  // 响应信息
  responseStatus?: number;
  durationMs?: number;
  responseHeaders?: Record<string, string>;
  error?: string;
}
```

### 3. 配置 (Config)

```typescript
interface Config {
  listen: {
    host: string;
    port: number; // 代理端口 7070
  };

  api: {
    host: string;
    port: number; // API端口 7071
  };

  upstreams: {
    anthropic: string;
    openai: string;
    gemini: string;
  };

  rules: Rule[];

  storage: {
    maxHistory: number; // 100
    autoCleanup: boolean; // true
    cleanupInterval: number; // 小时
  };

  debug: boolean;
}
```

---

## 🔄 核心流程

### 流程1：规则编辑与生效

```
用户在前端编辑规则
  ↓
点击保存
  ↓
前端 POST /api/config/sync
  ↓
后端：
  1. 验证规则格式
  2. 更新内存中的config.rules
  3. 写入 ~/.local/promptxy/config.json
  4. 返回成功
  ↓
前端显示保存成功
  ↓
下一个CLI请求到达代理(7070)
  ↓
代理使用新规则修改请求
  ↓
完成
```

### 流程2：请求捕获与展示

```
CLI发起请求 → 代理(7070)
  ↓
代理捕获请求：
  1. 记录原始请求体
  2. 应用规则 → 得到修改后请求
  3. 转发到上游API
  4. 获取响应
  5. 记录到SQLite
  6. 通过SSE推送事件
  ↓
前端SSE客户端收到事件
  ↓
前端 GET /api/requests/:id
  ↓
显示请求详情（原始 vs 修改对比）
```

### 流程3：数据清理

```
后端定时任务（每小时）：
  ↓
检查数据库记录数
  ↓
如果 > 100条
  ↓
DELETE FROM requests
WHERE id NOT IN (
  SELECT id FROM requests
  ORDER BY timestamp DESC
  LIMIT 100
)
  ↓
记录清理日志
```

---

## 🎨 前端页面设计（HeroUI）

### 页面1：规则列表 (RulesPage)

**组件结构：**

```
<Page>
  <Header title="规则管理" actions={[新建, 批量操作]} />

  <SearchAndFilter>
    <Input placeholder="搜索规则..." />
    <Select label="客户端" options={[全部, Claude, Codex, Gemini]} />
    <Select label="状态" options={[全部, 启用, 禁用]} />
  </SearchAndFilter>

  <Stats cards={[
    { label: '总数', value: 42 },
    { label: '启用', value: 38, color: 'success' },
    { label: '禁用', value: 4, color: 'error' }
  ]} />

  <RuleList>
    {rules.map(rule => (
      <RuleCard
        key={rule.id}
        rule={rule}
        onEdit={handleEdit}
        onDelete={handleDelete}
        onToggle={handleToggle}
      />
    ))}
  </RuleList>

  <Pagination />
</Page>
```

**HeroUI组件使用：**

- `@heroui/input` - 搜索框
- `@heroui/select` - 筛选器
- `@heroui/card` - 规则卡片
- `@heroui/button` - 操作按钮
- `@heroui/modal` - 编辑弹窗
- `@heroui/pagination` - 分页

### 页面2：请求详情 (RequestDetail)

**组件结构：**

```
<Modal open={true} onClose={close}>
  <Modal.Header>
    <Text h3>请求详情</Text>
  </Modal.Header>

  <Modal.Body>
    <Grid>
      <Grid xs={12}>
        <Card>
          <Card.Body>
            <Description
              data={{
                ID: request.id,
                时间: formatTime(request.timestamp),
                客户端: request.client,
                路径: request.path,
                状态: request.responseStatus
              }}
            />
          </Card.Body>
        </Card>
      </Grid>

      <Grid xs={6}>
        <Card>
          <Card.Header>原始请求</Card.Header>
          <Card.Body>
            <CodeBlock language="json">
              {JSON.stringify(request.originalBody, null, 2)}
            </CodeBlock>
          </Card.Body>
        </Card>
      </Grid>

      <Grid xs={6}>
        <Card>
          <Card.Header>修改后请求</Card.Header>
          <Card.Body>
            <CodeBlock language="json">
              {JSON.stringify(request.modifiedBody, null, 2)}
            </CodeBlock>
          </Card.Body>
        </Card>
      </Grid>

      <Grid xs={12}>
        <Card>
          <Card.Header>匹配规则</Card.Header>
          <Card.Body>
            <List>
              {request.matchedRules.map(match => (
                <List.Item key={match.ruleId}>
                  {match.ruleId} ({match.opType})
                </List.Item>
              ))}
            </List>
          </Card.Body>
        </Card>
      </Grid>
    </Grid>
  </Modal.Body>

  <Modal.Footer>
    <Button onPress={replay}>重放</Button>
    <Button onPress={export}>导出</Button>
    <Button onPress={close}>关闭</Button>
  </Modal.Footer>
</Modal>
```

---

## 📝 开发任务清单

### 阶段1：后端实现

- [ ] 创建 backend 目录结构
- [ ] 初始化 TypeScript 配置
- [ ] 安装依赖 (express, sqlite3, etc.)
- [ ] 实现 database.ts (SQLite初始化、CRUD)
- [ ] 实现 config.ts (配置读写)
- [ ] 扩展 gateway.ts (添加请求记录、SSE推送)
- [ ] 实现 api-server.ts (所有API端点)
- [ ] 修改 main.ts (启动两个服务器)
- [ ] 测试API端点

### 阶段2：前端初始化

- [ ] 创建 frontend 目录结构
- [ ] 初始化 Vite + React + TypeScript
- [ ] 安装 HeroUI 及依赖
- [ ] 配置 Tailwind CSS
- [ ] 创建基础路由结构
- [ ] 实现 API 客户端
- [ ] 实现 SSE 客户端
- [ ] 创建 Zustand stores

### 阶段3：前端页面实现

- [ ] RulesPage (列表、编辑、删除)
- [ ] RequestsPage (列表、筛选、分页)
- [ ] RequestDetail (模态框、左右对比)
- [ ] PreviewPage (测试沙盒)
- [ ] SettingsPage (配置管理)

### 阶段4：集成与部署

- [ ] 前后端联调测试
- [ ] 开发环境脚本 (dev.sh)
- [ ] 构建脚本 (build.sh)
- [ ] 生产部署配置
- [ ] Docker支持 (可选)
- [ ] 文档完善

---

## 🚀 快速开始

### 开发环境

**1. 启动后端**

```bash
cd backend
npm install
npm run dev  # 监听模式，端口7070 + 7071
```

**2. 启动前端**

```bash
cd frontend
npm install
npm run dev  # Vite开发服务器，默认5173
```

**3. 配置代理**

```bash
# Claude Code
export ANTHROPIC_BASE_URL="http://127.0.0.1:7070"

# Codex CLI
export OPENAI_BASE_URL="http://127.0.0.1:7070/openai"

# Gemini CLI
export GOOGLE_GEMINI_BASE_URL="http://127.0.0.1:7070/gemini"
```

**4. 访问Web UI**

```
http://localhost:5173
```

### 生产部署

```bash
# 1. 构建前端
cd frontend
npm run build

# 2. 复制到后端静态目录
cp -r dist/* ../backend/public/

# 3. 启动后端（会同时提供API和静态文件）
cd ../backend
npm start
```

访问：`http://localhost:7071`

---

## 🎯 关键设计决策

### 1. 为什么分开开发但统一部署？

- **开发体验**：前端热重载，后端独立重启
- **部署简单**：最终一个进程，一个端口
- **维护方便**：清晰的代码分离

### 2. 为什么使用SSE而不是WebSocket？

- **单向通信**：只需要服务器推送新请求
- **实现简单**：原生支持，无需额外库
- **资源友好**：连接数限制更少

### 3. 为什么使用SQLite而不是文件JSON？

- **查询能力**：支持分页、筛选、索引
- **并发安全**：避免文件锁问题
- **性能**：100条记录查询毫秒级

### 4. 为什么使用HeroUI？

- **组件丰富**：表单、表格、模态框等开箱即用
- **美观专业**：符合开发者工具审美
- **TypeScript友好**：完整类型支持
- **Tailwind集成**：易于自定义

---

## 📚 相关文档

- [后端API扩展设计](./BACKEND_API_EXTENSION.md)
- [前端HeroUI配置指南](./FRONTEND_SETUP.md) (待创建)
- [部署指南](./DEPLOYMENT.md) (待创建)

---

**文档版本**: v2.0
**创建日期**: 2025-12-20
**状态**: ✅ 已实现 (2025-12-21)
