...
- [ ] 0.1 复核并冻结对齐文档（作为实现验收依据，不再“边写边改规格”）
  - `docs/transformers/transformer-refactor.md`
  - `docs/protocols/claude-messages-spec.md`
  - `docs/protocols/codex-responses-spec.md`
- [ ] 0.2 明确 Open Questions 的决策（写入 `design.md` 的 Decision，或写成明确的 v1 fallback）
  - Claude SSE 的 `message_delta.delta.stop_reason`：tool call 场景是否需要特殊值？
  - `custom_tool_call` 的 Claude 侧 input 映射策略：包一层 object 还是直接 error？
  - FieldAudit 落库体积控制策略：截断/采样边界与保真要求
...
- [ ] 1.1（backend）盘点旧 transformers 对外 API 与调用点（作为新 public API 设计输入）
...
- [ ] 1.4（backend）让网关与 preview 重新跑起来（即使功能未完整，也必须能给出可定位错误）
...
- [ ] 2.1（backend）定义 FieldAudit（JSON Pointer 路径标准）与最小收集器
...
- [ ] 2.2（backend）扩展 transform trace / preview 输出：携带 FieldAudit 摘要与关键 evidence
...
- [ ] 3.1（backend）实现 `instructions = template + normalize(system)`（并记录 defaulted/mapped/unmapped）
- [ ] 3.2（backend）实现 messages 映射：text blocks → `input[].type="message"`（与现有行为对齐）
- [ ] 3.3（backend）实现 `tool_use` → `function_call`（`arguments` 必须为 JSON string）
- [ ] 3.4（backend）实现 `tool_result` → `function_call_output`
  - v1 保守策略：非 string 的 content 统一 JSON.stringify
  - FieldAudit 记录 `outputWasStringified` 与原始路径
- [ ] 3.5（backend）实现 call_id 配对与对称性校验（缺失/重复/孤儿 → error）
- [ ] 3.6（backend）将校验失败映射为可定位的错误响应（错误类型 + missingPaths/invariant + stepName）

## 4. SSE 转换：Codex Responses SSE → Claude SSE（Claude Code 可消费）

- [ ] 4.1（backend）实现 Codex Responses SSE 解析：以 `data.type` 分发（不依赖 `event:` 行）
- [ ] 4.2（backend）实现最小状态机：text delta 与 tool call 并存，按 `item.type` 处理 output_item.done
- [ ] 4.3（backend）实现 tool call SSE 映射：
  - `function_call/custom_tool_call` → `content_block_start` → `input_json_delta` → `content_block_stop` → `message_delta`
- [ ] 4.4（backend）实现 stream 结束规则：
